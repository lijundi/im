...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query(value = "select * from chat_message where ((from_id = ?1 and to_id = ?2) or (from_id = ?2 and to_id = ?1))  and time_stamp < ?3 ORDER BY time_stamp ASC limit 10", nativeQuery = true)
