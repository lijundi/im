...
```json
{
    ...
    "data":[
        ...
        {
            "type":"bp",
            "content":"您该测量血压了，请及时测量！",
            "timeStamp":"2020-12-10 16:24:01"
        },
        {
            "type":"tocda",
            "content":"您该填写近期症状评估表单了，请及时填写！",
            "timeStamp":"2020-12-10 16:24:01"
        }
    ]
}
...
> 根据时间戳获取系统发送给用户的历史消息，每次返回至多10条记录
...
| 字段名 | 类型            | 描述                               |
| ------ | --------------- | ---------------------------------- |
| -      | array of sysMsg | 返回历史系统消息列表，至多10条记录 |

##### sysMsg

> 参见消息-离线系统消息-返回参数-sysMsg
...
> 获取指定目录的下级列表，返回列表为子目录或者问题；若要获取主目录，指定目录id为0
...
| dbp                  | int    | 舒张压               |
| heart_rate           | int    | 心率                 |
| create_time          | string | 测量时间             |
| is_exercise          | string | 是否运动后：是/否    |
| exercise_mode        | string | 运动方式             |
| exercise_km          | string | 运动公里数           |
| exercise_min         | string | 运动持续时长（分钟） |
| is_get_up            | string | 是否刚睡醒：是/否    |
| bedtime              | string | 睡觉时间             |
| sleep_hour           | int    | 睡觉时长（小时）     |
| wake_up_num          | int    | 醒来次数             |
| is_discomfort        | string | 是否感觉不适：是/否  |
| detail_of_discomfort | string | 不适详情             |
    ...
    "data":[
        {
            ...
            "dbp":90,
            "heart_rate":90,
            "create_time":"2020-10-10 21:51:57",
            "is_exercise":"否",
            "exercise_mode":null,
            "exercise_km":null,
            "exercise_min":null,
            "is_get_up":"是",
            "bedtime":"2020-10-10 21:51:57",
            "sleep_hour":6,
            "wake_up_num":0,
            "is_discomfort":"是",
            "detail_of_discomfort":"胸闷"
    	},
        {
            "id":2,
            "userId":"xxxxxx1",
...
| dbp                  | int    | 是       | 舒张压               |
| heart_rate           | int    | 否       | 心率                 |
| create_time          | string | 否       | 测量时间             |
| is_exercise          | string | 否       | 是否运动后：是/否    |
| exercise_mode        | string | 否       | 运动方式             |
| exercise_km          | string | 否       | 运动公里数           |
| exercise_min         | string | 否       | 运动持续时长（分钟） |
| is_get_up            | string | 否       | 是否刚睡醒：是/否    |
| bedtime              | string | 否       | 睡觉时间             |
| sleep_hour           | int    | 否       | 睡觉时长（小时）     |
| wake_up_num          | int    | 否       | 醒来次数             |
| is_discomfort        | string | 否       | 是否感觉不适：是/否  |
| detail_of_discomfort | string | 否       | 不适详情             |
